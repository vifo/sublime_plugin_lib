...
class SublimePluginLibShowScratchViewCommand(sublime_plugin.TextCommand):
    ...
    """
    Sublime Text text command ``sublime_plugin_lib_show_scratch_view``. Opens
    a new scratch view with given contents.
    """
        ...
        for k in ['header', 'name', 'syntax']:
            if k in kwargs:
                if not isinstance(kwargs[k], compat.string_types):
                    raise ValueError(
                        'Option "%s" passed to run() must be a string' % (k))
                options[k] = kwargs[k]
        ...
        for k in ['spell_check', 'scratch']:
            if k in kwargs:
                if not isinstance(kwargs[k], bool):
                    raise ValueError(
                        'Option "%s" passed to run() must be a boolean' % (k))
                options[k] = kwargs[k]
